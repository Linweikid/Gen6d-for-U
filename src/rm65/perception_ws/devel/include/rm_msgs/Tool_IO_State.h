...
struct Tool_IO_State_
{
  ...
  Tool_IO_State_()
    : Tool_IO_Mode()
    , Tool_IO_State()  {
      Tool_IO_Mode.assign(0);

      Tool_IO_State.assign(0);
  }
  ...
  (void)_alloc;
   ...
   typedef boost::array<int8_t, 2>  _Tool_IO_Mode_type;
  _Tool_IO_Mode_type Tool_IO_Mode;

   typedef boost::array<int8_t, 2>  _Tool_IO_State_type;
...
struct MD5Sum< ::rm_msgs::Tool_IO_State_<ContainerAllocator> >
{
  ...
  static const uint64_t static_value1 = 0x3a8d67c6456299e6ULL;
  static const uint64_t static_value2 = 0x5ebafb60fec4550bULL;
};
...
struct Definition< ::rm_msgs::Tool_IO_State_<ContainerAllocator> >
{
  static const char* value()
  {
    return "int8[2] Tool_IO_Mode          #数字I/O输入/输出状态  0-输入模式，1-输出模式\n"
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::rm_msgs::Tool_IO_State_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.Tool_IO_Mode.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.Tool_IO_Mode[i]);
    }
    if (v.Tool_IO_Mode.empty() || true)
      s << "]";
    ...
    for (size_t i = 0; i < v.Tool_IO_State.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.Tool_IO_State[i]);
    }
    if (v.Tool_IO_State.empty() || true)
      s << "]";
  }
